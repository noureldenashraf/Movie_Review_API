...
public class MoviesService {
    ...
    private DirectorRepository directorDao;
    private ReviewRepository reviewDao;

    @Autowired
    public MoviesService(MoviesRepository movie, DirectorRepository director, ReviewRepository review) {
    ...
    public ResponseEntity<?> getAllGenres () {
        ...
        if (allGenres.isEmpty()){return ResponseEntity.badRequest().body("No genres found");}
    ...
    public ResponseEntity<?> getAllMoviesWithGenre(Optional<String> genreName){
        if (genreName.isEmpty()){return ResponseEntity.badRequest().body("Put genre name in Path");}
        List<?> allMoviesWithGenre = moviesDao.getAllMoviesWithGenre(genreName.get());
        if(allMoviesWithGenre.isEmpty()){return ResponseEntity.badRequest().body("No movies with this genre name, make sure to write them correct , here is a list of all genre "+ moviesDao.getALlGenres());}
